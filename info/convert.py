...
def alpha_2(country_name):
        '''
        NOT : "İ" ile başlayan ülkelerin ilk harfini büyük yazın.(örneğin: İspanya, İran)
        Diğer ülkeleri küçük harfle de yazabilirsiniz.
        istekte bulunduğumuz api ülke verilerini alpha-2 koduna göre tutuyor.(örneğin: Türkiye = TR)
        ulkeler.json dosyasında ülkelerin Türkçe isimleri ve alpha-2 kodlarını dictionary tipinde tutuluyor
        Bu fonksiyonda yazdığınız ülkenin alpha-2 kod karşılığını döndürür.
...
def date_convert(date):
    return parser.isoparse(date).astimezone(pytz.timezone("Europe/Istanbul"))
...
def fetch(*args):
    ...
    raise NameError(f'{code} koduna sahip ülkeye ait veri bulunamadı')
...
# Convert string to datetime object, then format to Europa/Istanbul
